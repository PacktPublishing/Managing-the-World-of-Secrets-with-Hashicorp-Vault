...
sudo scpi "vault-kp1.pem" root_2024_ca.crt ubuntu@ec2-18-119-162-200.us-east-2.compute.amazonaws.com:/tmp

# After you SSH into the instance
cpp /tmp/root_2024_ca.crt /opt/vault/tls/ 
...
vault operator diagnoseconfig= /etc/vault.d/vault.hcl
...
chown vault:vaultR /opt/vault.d/
chown vault:vaultR /opt/vault.d*
...
vault secrets enableversion=2 kv



vault operator initkey-shares=1key-threshold=1
...
vault operator raft joinleader-ca-cert=@/opt/vault/tls/root_2024_ca.crtleader-client-cert=@/opt/vault/tls/tls.crtleader-client-key=@/opt/vault/tls/tls.key <https://load-balacner-IP:8200>
