...
# Tune it for 5
vault secrets tunemax-lease-ttl=87600h pki 

# Generate Root CA certificate
vault writefield=certificate pki/root/generate/internal \ 
...
vault secrets enablepath_pki_int pki

# Tune for 3 eyars
vault secrets tunemax-lease-ttl=26280h pki

# Sign the CSR with the Root CA's private key
vault writeformat=json pki_int/intermediate/generate/internal \ 
    common_name="<your_org.com> Intermediate Authority" | 
    | jqr '.data.csr' > pki_intermediate.csr

# Generate Intermediate CA certificate
vault writeformat=json pki/root/sign-intermediate csr=@pki_intermediate.csr \ 
    format=pem_bundle ttl="43800h" \ 
    | jqr '.data.certificate' > intermediate.cert.pem
