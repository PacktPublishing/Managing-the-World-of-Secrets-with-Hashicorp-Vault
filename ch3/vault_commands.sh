...
# ExecStart=/usr/bin/vault serverconfig=/etc/vault.d/vault.hcl
/usr/bin/vault serverconfig=/etc/vault.d/vault.hcl


vault operator init

vault token createpolicy=policy1policy=policy2
...
vault token createpolicy=corp1-dept1-and-2                                       
# Key                  Value
#--                 ----
...
vault token createpolicy=token_creationttl=10m
# Key                  Value
#--                 ----
...
vault listformat json auth/token/accessors | \ 
    jqr .[] | xargsI '{}' vault token lookupformat jsonaccessor '{}' | \ 
    jqr 'select(.data.policies | any(. == "root"))'
...
vault token lookupaccessor XcFFLRGJivUxQSYTgjSqOWCV                        
# Key                 Value
#--                ----
# accessor            XcFFLRGJivUxQSYTgjSqOWCV
...
vault token createorphanpolicy my-policyttl 30m

# Create a token that has sudo access to auth/token/create
vault token createpolicy=sudo-access_to_auth-token-create                       
# Key                  Value
#--                 ----
...
vault loginmethod=userpass username=tokenman1 password=pwd123                                          
...
vault token lookupaccessor UUlC4fNGEddPv7hxhwDx7Tj8
# Key                 Value
#--                ----
...
openssl req -new -newkey rsa:2048 -nodes -keyout vault.key -out vault.csr -subj '/CN=localhost' && openssl x509 -req -days 365 -in vault.csr -signkey vault.key -out vault.crt
